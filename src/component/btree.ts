...
// Max operations applied in a single transaction, so we stay within Convex's
// per-transaction read/write limits. A batch is applied whole in one
// transaction, so this caps a batch's size: enqueueOperations and
// applyOperations reject anything larger, ensuring every batch can be drained.
const MAX_OPS_PER_BATCH = 100;

// Max operations one worker cycle drains across batches. processBatch applies
// all of a cycle's batches in a single transaction, so this bounds that
// transaction; it must be >= MAX_OPS_PER_BATCH so a single batch always fits.
...
export async function enqueueOperations(
  ...
  if (operations.length > MAX_OPS_PER_BATCH) {
    throw new Error(
      `Cannot enqueue a batch of ${operations.length} operations; the maximum ` +
        `is ${MAX_OPS_PER_BATCH}. Split the writes across multiple batches.`,
    );
  }
...
// Apply a list of operations. Shared by the synchronous `batch` mutation and the
// worker's `processBatch`. Each handler fetches its own tree at execution time
// rather than reusing a cached doc: any operation can patch `btree.root` (a root
// split on insert, a root collapse on delete), which would leave a cached
...
export async function applyOperations(
  ...
  if (operations.length > MAX_OPS_PER_BATCH) {
    throw new Error(
      `Cannot apply a batch of ${operations.length} operations in one ` +
        `transaction; the maximum is ${MAX_OPS_PER_BATCH}.`,
    );
  }
