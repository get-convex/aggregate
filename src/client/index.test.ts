...
describe("lazy reads", () => {
  test("a lazy read does not see writes made in the current transaction", async () => {
    const t = setupTest();
    const { aggregate } = createAggregates();

    const count = await t.run(async (ctx) => {
      await aggregate.insert(ctx, await testItem(ctx, { name: "a", value: 1 }));
      return await aggregate.count(ctx, { lazy: true });
    });
    expect(count).toBe(0);
  });

  test("a non-lazy read does see writes made in the current transaction", async () => {
    const t = setupTest();
    const { aggregate } = createAggregates();
