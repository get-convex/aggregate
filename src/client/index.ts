...
> {
  ...
  startBuffering(opts?: { stale?: boolean }): void {
    ...
    // Changing the stale mode mid-buffer would apply already-queued operations
    // via the wrong path on flush. Reject the transition until the queue drains.
    if (
      this.isBuffering &&
      this.operationQueue.length > 0 &&
      stale !== this.bufferStale
    ) {
      throw new Error(
        "Cannot change buffer stale mode while operations are queued; " +
          "flush or finishBuffering first.",
      );
    }
    this.isBuffering = true;
    this.bufferStale = stale;
  }
  ...
  async flush(
    ...
    const flushPromise = ctx.runMutation(flushMutation, { operations });
    this.currentFlushPromise = flushPromise;
    // end critical section
    try {
      await flushPromise;
    } finally {
      // Clear on both resolve and reject so a failed batch doesn't leave the
      // rejected promise latched forever.
      if (this.currentFlushPromise === flushPromise) {
