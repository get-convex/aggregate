...
export type BarValue = {
  ...
  high?: number;
  /**
   * Overrides the series color for this bar. For a chart grouped by mode the
   * mode is the group, and it's the group that owns the color.
   */
  color?: string;
};
...
export type GroupedBarChartProps = {
  ...
  ariaLabel: string;
  /**
   * Width of a label column naming each series beside its bar. Set when color
   * doesn't identify the series — the bars within a group share the group's
   * color — so a legend couldn't either. Omit for the usual one-color-per-series
   * layout, where the legend does that job.
   */
  seriesLabelWidth?: number;
};
        ...
        >
          ...
          <Stack gap={MARK.surfaceGap}>
            {series.map((s) => {
                ...
                >
                  <Box
                    style={{
                      ...
                      left: 0,
                      width: `${barPct}%`,
                      background: entry.color ?? s.color,
              ...
              if (seriesLabelWidth === undefined) {
                return <Box key={s.label}>{bar}</Box>;
              }
              return (
                <Box
                  key={s.label}
                  style={{
                    display: "grid",
                    gridTemplateColumns: `${seriesLabelWidth}px minmax(0, 1fr)`,
                    alignItems: "center",
                    columnGap: 8,
                  }}
                >
                  {/* Right-aligned, so every label ends at the bar baseline. */}
                  <Text
                    size="xs"
                    c={INK.muted}
                    ta="right"
...
function Whisker({ lowPct, highPct }: { lowPct: number; highPct: number }) {
  ...
  return (
    ...
    >
      <Cap />
      <Box
        style={{ flex: 1, height: 1, background: INK.primary, opacity: 0.85 }}
      />
      <Cap />
    </Box>
  );
