...
describe("shuffle", () => {
  ...
  test("shuffle", async () => {
    ...
    const idToDelete = await t.mutation(api.shuffle.addMusic, { title: "Song4" });
    ...
    const shufflePage0 = await t.query(api.shuffle.shufflePaginated, { offset: 0, numItems: 3, seed: "" });
    expect(shufflePage0).toEqual(["Song6", "Song1", "Song3"]);
    const shufflePage1 = await t.query(api.shuffle.shufflePaginated, { offset: 3, numItems: 3, seed: "" });
    expect(shufflePage1).toEqual(["Song5", "Song2"]);
    
    // With different seed, we should get a different shuffle
    const shufflePage0Seed1 = await t.query(api.shuffle.shufflePaginated, { offset: 0, numItems: 3, seed: "x" });
    expect(shufflePage0Seed1).toEqual(["Song1", "Song6", "Song5"]);
    const shufflePage1Seed1 = await t.query(api.shuffle.shufflePaginated, { offset: 3, numItems: 3, seed: "x" });
